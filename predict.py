...
def translate_to_origin(model, text, language):
    ...
    char_to_index, index_to_char, vocab_size, trans_to_index, _, trans_vocab_size = utils.load_vocabulary(language)
...
def main():
    ...
    if not args.model:
        print("--model must be defined, pass model path to load")
        return
    ...
    if args.text:
        # print("--text must be passed to predict translation of it")
        text, char_list = translate_to_origin(model, args.text, args.language)
        ...
        while True:
            x = input('Enter text for translate, or "Exit" :')
            if x.lower() == 'exit':
                return
